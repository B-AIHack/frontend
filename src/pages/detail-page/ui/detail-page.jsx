...
export const DetailPage = () => {
  ...
  const { data: detail, isFetching } = useDetailQuery(params?.id)
  ...
  const renderStatusTag = () => {
    const iconLeftMap = {
      ...
      ['FINISHED']: ErrorCircleColoredIcon
    }
    ...
    const Icon = iconLeftMap[detail?.data?.status]

    return (
      <Tag
        iconLeft={() => <Icon size='s' />}
        variant='secondary'
        color={colorMap[detail?.data?.status]}
  ...
  return (
    <div>
      <Stack fullWidth justify='spaceBetween' className={spacing({ mb: 'l' })}>
        ...
        <ExecuteButton disabled={!isNew} id={params?.id} />
      </Stack>
